...
class ComboBox(wx.ComboBox):
    ...
    def _bind(self, controller):
        obj = self
        self.Bind( wx.EVT_COMBOBOX, lambda event, obj=obj: controller.on_combo(event, obj) )
...
class TextCtrl(wx.TextCtrl):
    ...
    def _bind(self, controller):
        obj = self
        self.Bind( wx.EVT_TEXT, lambda event, obj=obj: controller.on_textctrl(event, obj) )
        self.Bind( wx.EVT_TEXT_ENTER, lambda event, obj=obj: controller.on_textctrl(event, obj) )
...
class Controller(object):
    ...
    def __init__(self, model, view):
        #if model is a module, load module
        if hasattr(model, '__name__'):
            self.model = importlib.import_module(model.__name__)
        else:
            self.model = model
